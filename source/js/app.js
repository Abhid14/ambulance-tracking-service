...
firebase.auth().onAuthStateChanged(function (user) { // realtime authenctication listener
    // if it is logged in 
    if (user) {
        ...
        navigator.geolocation.getCurrentPosition((pos) => {
            ...
            db.collection("runningops")
                ...
                .then(() => {
                    ...
                    var rtl, rtloptions;
                    rtloptions = { enableHighAccuracy: true, timeout: 3500, maximumAge: 0 };
...
function addDetUI(usrDet) {
    ...
    $$(".runOPSItem").on("click", function () { // realtime click event listener
        folUsr(this.id);
    });
    ...
    if (pty == "color-green") {
        app.dialog.alert(
            "A Green corridor vehicle " + userDet[2] + " has been detected in your 4 km range!",
            "Important Alert!"
        );
    }
}
